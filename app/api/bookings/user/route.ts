...
export async function POST(request: NextRequest) {
    try {
        ...
        const { tripId, email, phone, passengers, paymentReference, reference } = body;
        ...
        console.log('Reference truthy?', !!reference);
        ...
        // Use the provided reference (from payment page/webhook) or generate a new one
        let bookingReference: string;
        if (reference && typeof reference === 'string' && reference.trim()) {
            bookingReference = reference.trim();
            console.log('Using provided reference:', bookingReference);
        } else {
            bookingReference = `TE${uuidv4().slice(0, 8).toUpperCase()}`;
            console.log('Generated new reference:', bookingReference);
        }

        // Check if booking reference already exists
        if (reference && typeof reference === 'string' && reference.trim()) {
            const existingBookingByReference = await prisma.booking.findFirst({ where: { reference: bookingReference } });
            if (existingBookingByReference) {
                return NextResponse.json({ error: { code: 400, message: 'Booking reference already exists' } }, { status: 400 });
            }
        }
        ...
        const booking = await prisma.$transaction(async (tx) => {
            const createdBooking = await tx.booking.create({
                data: {
                    reference: bookingReference, // Use the reference from payment page/webhook
                    status: 'confirmed',
                    tripId,
                    busId: trip.busId,
                    from: trip.from,
